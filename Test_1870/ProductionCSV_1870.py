...
def process_portrait_files(xml_file_name, xml_file_path):
        try:
            print ("ca marche pour :")
            print(xml_file_name)
            # Lire et analyser le fichier XML
            tree = ET.parse(xml_file_path)
            root = tree.getroot()
            ...
            # Liste pour stocker le contenu extrait
            extracted_content = []

            # Extraire les éléments avant la moitié verticale
            for string_element in string_elements:
                content = string_element.get('CONTENT')
                hpos = int(string_element.get('HPOS', 0))

                # Ne prendre en compte que les éléments avant la moitié de la page (selon HPOS)
                if hpos > moitie_image or re.match(r'.*\d+.*', content) or re.match(r'[A-Z]{3,}', content) or content in exclusion_terms :
                    pass
                else : 
                    extracted_content.append(content)
                                
            # Génération d'un fichier .txt avec le contenu extrait
            generate_portrait_txt_output(xml_file_name, extracted_content)

        except Exception as e:
            print(f"Erreur lors du traitement du fichier en portrait {xml_file_name}: {e}")

# Fonction pour générer des fichiers .txt pour les fichiers en portrait
def generate_portrait_txt_output(xml_file_name, extracted_content):
    txt_file_path = os.path.join(txt_output_folder, f"portrait_{xml_file_name.replace('.xml', '')}.txt")
    with open(txt_file_path, 'w', encoding='utf-8') as f:
